  ...
  `;
    ...
    // Fetch job positions for the company to resolve job titles by id
    ...
    const handlePreview = () => {
      ...
      // Normalize recipients similar to send flow so we can preview per-recipient
      const normalizedRecipients = recipients
        .map((item) => {
          if (typeof item === 'string') {
            return { to: item, applicant: undefined as string | undefined, jobPositionId: undefined as string | undefined, raw: item };
          }
      ...
      // Single recipient: show single email preview with substitutions
      ...
      // Multiple recipients: create combined preview document with a section per recipient
      const sections = normalizedRecipients.map((nr) => {
        ...
        // subject inside section should be escaped for HTML
  ...
  try {
    ...
    const normalizedRecipients = recipients
      .map((item) => {
        if (typeof item === 'string') {
          // For string items, create a simple object with just email
          const email = item.trim();
          return { 
            to: email, 
            applicant: undefined, 
            jobPositionId: undefined,
            raw: { email } // Pass the raw object for substitution
          };
        }
        ...
        // Create a raw object that contains all the necessary fields for substitution
        ...
        return {
          ...
          jobPositionId: typeof jobPositionId === 'string' ? jobPositionId : undefined,
          applicantName: item?.applicantName || item?.name || item?.fullName,
          raw: rawForSubstitution, // Store the full object for substitution
        };
      })
      .filter((item) => item.to);
    ...
    const batch = normalizedRecipients.map(({ to, applicant, jobPositionId, raw }) => {
      // Apply substitutions for this specific recipient
      ...
      console.log(`Processing recipient: ${to}`);
      console.log('Substituted subject:', subSubject);
      console.log('Substituted body preview:', subBody.substring(0, 200));
    ...
    await Swal.fire({ 
      title: 'Success', 
      text: `Email queued for ${recipients.length} recipient(s)`, 
      icon: 'success', 
      timer: 2000, 
