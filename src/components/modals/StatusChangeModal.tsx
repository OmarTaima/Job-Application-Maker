...
export default function StatusChangeModal({
      ...
      if (
        ...
        return statusOptions.filter((option: any) => {
          ...
          return job.allowedStatuses.some((allowedId: any) => {
            const normalizedAllowedId = String(
              allowedId?._id || allowedId?.id || allowedId || ''
            )
              .trim()
              .toLowerCase();
            const normalizedOptionValue = String(option.value || '')
              ...
              .toLowerCase();
            return (
              normalizedOptionValue === normalizedAllowedId ||
              option.text?.toLowerCase() === normalizedAllowedId
            );
          });
        });
      }
  ...
  const handleStatusSelect = (value: any) => {
    ...
    const isRejectedStatus =
      selectedValue && selectedValue.toLowerCase() === 'rejected';
  ...
  const handleReasonsChange = (selected: string[]) => {
    ...
    const newCustomReasons = selected.filter(
      (reason) => !originalOptions.includes(reason)
    );
            ...
            {isRejected && (
              <div className="rejected-reasons-section p-4 bg-red-50 dark:bg-red-900/10 rounded-lg border border-red-200 dark:border-red-800">
                ...
                <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                  {reasonOptions.length} total reasons • Type to filter • Press
                  Enter to add new reason • Click to select
                </p>

                {selectedValues.length > 0 && (
                  <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    {selectedValues.length} reason
