...
const interviewApplicants = () => {
  ...
  // Only interview applicants — the sole dataset for this page
  const interviewApplicants = useMemo(
    () => (applicants as Applicant[]).filter((a: any) => a.status === 'interview'),
    [applicants]
  );
  const deferredinterview = useDeferredValue(interviewApplicants);
  ...
  const tableData = isTableLoading ? skeletonData : deferredinterview;
  ...
  const mobileFiltered = useMemo(() => {
    try {
      const list = Array.isArray(deferredinterview) ? deferredinterview.slice() : [];
    ...
    } catch (e) { return Array.isArray(deferredinterview) ? deferredinterview : []; }
  }, [deferredinterview, mobileQuery]);
  ...
  const table = useMaterialReactTable({
    ...
    manualSorting: false,
    rowCount: isTableLoading ? tableData.length : deferredinterview.length,
    initialState: {
      pagination,
  ...
  return (
    <>
      <PageMeta title="interview Applicants" description="View interview applicants and their reasons" />
      <PageBreadcrumb
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-red-100 dark:bg-red-900/30 px-3 py-1 text-sm font-semibold text-red-700 dark:text-red-300">
              ...
              {isTableLoading ? '…' : deferredinterview.length} interview
            </span>
      ...
      <div className="grid gap-6">
        <ComponentCard title="Interview Applicants" desc="All applicants with interview status, showing interview details from their status history">
          <>
            {error && (
