...
// Types
import { MaterialReactTable, MRT_SelectCheckbox, useMaterialReactTable, type MRT_ColumnDef } from 'material-react-table';
...
const APPLICANTS_DEFAULT_LAYOUT = {
  columnVisibility: { rejectionReasons: false }, // hidden by default
  columnSizing: {},
  columnOrder: [],
};
...
function ImageThumbnailComponent({ src, alt, onClick }: { src?: string | null; alt?: string; onClick?: () => void }) {
...
export default function Applicants({ layoutKey, defaultLayout, onlyStatus, companyIdOverride }: ApplicantsProps = {}) {
...
const persistedTableState = useMemo(() => {
  ...
  // React Query hooks
  const { data: jobPositions = [], isFetching: isJobPositionsFetching, isFetched: isJobPositionsFetched, refetch: refetchJobPositions } = useJobPositions(companyId);
  const { data: applicants = [], error, refetch: refetchApplicants, isFetching: isApplicantsFetching, isFetched: isApplicantsFetched } = useApplicants(companyId as any);
  ...
  const mailCountByApplicantId = useMemo(() => {
    ...
    (mailApiResponse?.data || []).forEach((mail: any) => {
      const applicantId = typeof mail.applicant === 'string'
        ? mail.applicant.trim()
...
const selectedCompanyFilterValue = useMemo((): string[] | string | null | undefined => {
...
const {
  ...
  const tableMinWidth = isNarrowDesktopViewport ? 820 : isLaptopViewport ? 980 : 1160;
...
const handleApplicantLinkClick = useCallback(
  (e: React.MouseEvent<HTMLAnchorElement>, row: any) => {
    ...
    ...layout.columnVisibility 
        ...
        statusFilter: currentFilters.columnFilters.find((f: any) => f.id === 'status')?.value,
        companyFilter: currentFilters.columnFilters.find((f: any) => f.id === 'companyId')?.value,
  ...
  const handleApplicantLinkAuxClick = useCallback((e: React.MouseEvent<HTMLAnchorElement>) => {
    e.stopPropagation();
  }, []);
...
// In the useEffect that restores table state, add a check to ensure data is loaded first
  ...
  useEffect(() => {
    ...
    const update = () => { if (mountedRef.current) setElapsed(formatRelative(lastRefetch)); };
    update();
  ...
  const getExpectedSalaryDisplay = useCallback((applicant: any): string => {
    const toText = (value: any): string => {
      ...
      if (typeof value === 'object') {
        const candidateKeys = ['expectedSalary', 'salary', 'amount', 'value', 'val', 'label', 'name', 'title', 'en', 'ar', 'text'];
    ...
    const directCandidates = [applicant?.expectedSalary, applicant?.expected_salary, applicant?.salaryExpectation, applicant?.desiredSalary];
    for (const candidate of directCandidates) {
      const text = toText(candidate);
      if (text) return text;
    }
    const responses = applicant?.customResponses || applicant?.customFieldResponses || {};
    const normalizeKey = (key: any) => String(key || '').replace(/[\s_-]+/g, '').toLowerCase();
    const expectedKeyMatchers = ['expectedsalary', 'salary', 'salaryexpectation', 'desiredsalary', 'الراتب', 'راتب', 'الراتبالمتوقع'];
    for (const [key, value] of Object.entries(responses || {})) {
      const normalized = normalizeKey(key);
      const isSalaryKey = expectedKeyMatchers.some((matcher) => normalized.includes(normalizeKey(matcher)));
  ...
  const parseComparableNumber = useCallback((value: any): number | null => {
    ...
    if (Array.isArray(value)) {
      const nums = value.map((item) => parseComparableNumber(item)).filter((n): n is number => typeof n === 'number' && Number.isFinite(n));
      if (!nums.length) return null;
      return Math.max(...nums);
    }
    if (typeof value === 'object') {
      const candidates = [(value as any).expectedSalary, (value as any).salary, (value as any).amount, (value as any).value, (value as any).val, (value as any).max, (value as any).min];
      const nums = candidates.map((item) => parseComparableNumber(item)).filter((n): n is number => typeof n === 'number' && Number.isFinite(n));
      if (nums.length) return Math.max(...nums);
      const nestedNums = Object.values(value).map((item) => parseComparableNumber(item)).filter((n): n is number => typeof n === 'number' && Number.isFinite(n));
  ...
  const getApplicantSScore = useCallback((applicant: any): number | null => {
    const parseAnswer = (value: any): boolean => {
      if (typeof value === 'boolean') return value;
      if (typeof value === 'number') return value > 0;
      if (typeof value === 'string') {
        const normalized = value.trim().toLowerCase();
        if (['true', '1', 'yes', 'y', 'accepted', 'met', 'pass', 'passed'].includes(normalized)) return true;
        if (['false', '0', 'no', 'n', 'rejected', 'failed', 'not met'].includes(normalized)) return false;
      }
      return Boolean(value);
    };
    const getApplicantSpecResponses = (): any[] => {
      if (Array.isArray(applicant?.jobSpecsWithDetails) && applicant.jobSpecsWithDetails.length) return applicant.jobSpecsWithDetails;
      if (Array.isArray(applicant?.jobSpecsResponses) && applicant.jobSpecsResponses.length) return applicant.jobSpecsResponses;
      if (Array.isArray(applicant?.jobSpecs) && applicant.jobSpecs.length) return applicant.jobSpecs;
      if (typeof applicant?.jobPositionId === 'object' && applicant.jobPositionId) {
        if (Array.isArray(applicant.jobPositionId.jobSpecsWithDetails) && applicant.jobPositionId.jobSpecsWithDetails.length) return applicant.jobPositionId.jobSpecsWithDetails;
        if (Array.isArray(applicant.jobPositionId.jobSpecsResponses) && applicant.jobPositionId.jobSpecsResponses.length) return applicant.jobPositionId.jobSpecsResponses;
        if (Array.isArray(applicant.jobPositionId.jobSpecs) && applicant.jobPositionId.jobSpecs.length) return applicant.jobPositionId.jobSpecs;
      }
      return [];
    };
    const getJobSpecs = (): any[] => {
      ...
      const source = mapped || (typeof rawJob === 'object' ? rawJob : undefined) || applicant?.jobPosition;
      if (source && Array.isArray(source.jobSpecsWithDetails) && source.jobSpecsWithDetails.length) return source.jobSpecsWithDetails;
      if (source && Array.isArray(source.jobSpecs) && source.jobSpecs.length) return source.jobSpecs;
    ...
    applicantResponses.forEach((entry: any) => {
      ...
      const answerRaw = entry.answer ?? entry.accepted ?? entry.isAccepted ?? entry.met ?? entry.match ?? entry.selected;
      const answer = parseAnswer(answerRaw);
      [entry.jobSpecId, entry.specId, entry._id, entry.id].map((id) => resolveAnyId(id)).filter(Boolean).forEach((id) => { answerById[id] = answer; });
    });
    let totalWeight = 0, acceptedWeight = 0, totalCount = 0, acceptedCount = 0;
    specs.forEach((spec: any) => {
      totalCount += 1;
      const rawWeight = Number(spec?.weight ?? 0);
      const weight = Number.isFinite(rawWeight) && rawWeight > 0 ? rawWeight : 0;
      totalWeight += weight;
      const specIds = [spec?.jobSpecId, spec?.specId, spec?._id, spec?.id].map((id) => resolveAnyId(id)).filter(Boolean);
      ...
      if (accepted === undefined) {
        const fallback = applicantResponses.find((_: any, idx: number) => idx === specs.indexOf(spec));
        if (fallback !== undefined) {
          const fallbackRaw = fallback?.answer ?? fallback?.accepted ?? fallback?.isAccepted ?? fallback?.met ?? fallback?.match ?? fallback?.selected;
          accepted = parseAnswer(fallbackRaw);
        } else {
          accepted = false;
        }
      }
      if (accepted) { acceptedCount += 1; acceptedWeight += weight; }
    });
    if (totalWeight > 0) return Math.round((acceptedWeight / totalWeight) * 100);
  ...
  const downloadCvForApplicant = useCallback(async (a: any) => {
    ...
    const resolveCvPath = (applicant: any): string | null => {
      const keys = ['cvFilePath', 'resumePath', 'cvUrl', 'resumeUrl', 'curriculumVitaePath'] as const;
  ...
  const handleExportToExcel = useCallback(async () => {
    if (selectedApplicantIds.length === 0) {
      await Swal.fire({ title: 'No Selection', text: 'Please select at least one applicant to export.', icon: 'warning', timer: 2000, showConfirmButton: false });
      return;
    }
    setIsExporting(true);
    try {
  ...
  }, [selectedApplicantIds, applicants, jobPositionMap, companyMap, getExpectedSalaryDisplay, getApplicantSScore]);

  const isTableLoading = Boolean(isJobPositionsFetching || isApplicantsFetching || isCompaniesFetching);
...
const rejectionReasonsOptions = useMemo(() => {
  ...
  },
 {
  ...
  },
  {
    ...
    size: 90,
    enableSorting: true,
    accessorFn: (row: any) => mailCountByApplicantId.get(String(row?._id || row?.id || '')) ?? 0,
  ...
  ...(showCompanyColumn ? [{
    ...
    enableColumnFilter: false,
    enableSorting: true,
    accessorFn: (row: any) => getApplicantCompanyId(row, jobPositionMap),
    Header: ({ column }: { column: any }) => (
      <ColumnMultiSelectHeader
        column={column}
        label="Company"
  ...
  }] : []),
  {
    ...
    Cell: ({ row }: { row: { original: any } }) => {
      ...
      const title = typeof job?.title === 'string' ? job.title : (job?.title?.en ?? jobOptions.find((o) => o.id === jobId)?.title ?? 'N/A');
      return (
        <a
          href={getApplicantHref(row)}
  ...
  Cell: ({ row }: { row: { original: any } }) => {
    ...
    const applicantCompanyId = getApplicantCompanyId(row.original, jobPositionMap);
...
},
{
    ...
    Header: ({ column, table }: { column: any; table: any }) => {
      ...
      return (
        <button onClick={toggle} className="flex items-center gap-1 text-sm font-medium" type="button" title={desc ? 'Newest' : 'Oldest'}>
          <span>Submitted</span>
...
], [columnSizeConfig, isLaptopViewport, duplicatesOnlyEnabled, isTableLoading, genderOptions, showCompanyColumn, companyOptions, jobOptions, statusFilterOptions, effectiveOnlyStatus, getStatusColor, formatDate, getExpectedSalaryDisplay, getApplicantSScore, parseComparableNumber, mailCountByApplicantId, jobPositionMap, companyMap, currentUserId]);
  ...
  const muiTheme = useMemo(() => createTheme({
    palette: {
      ...
      primary: { main: '#e42e2b' },
      background: { default: isDarkMode ? '#24303F' : '#FFFFFF', paper: isDarkMode ? '#24303F' : '#FFFFFF' },
      text: { primary: isDarkMode ? '#E4E7EC' : '#101828', secondary: isDarkMode ? '#98A2B3' : '#667085' },
  ...
  const skeletonData = useMemo(() => Array.from({ length: pagination.pageSize || 10 }).map((_, i) => ({ _id: `skeleton-${i}`, _skeleton: true })), [pagination.pageSize]);
  ...
  const table = useMaterialReactTable({
    ...
    displayColumnDefOptions: {
      'mrt-row-select': {
        ...
        Cell: ({ row, table }: any) => {
          ...
          return (
            <div className="flex items-center justify-center p-2" onClick={(e) => e.stopPropagation()} onAuxClick={(e) => e.stopPropagation()}>
    ...
    initialState: {
      ...
      columnFilters,
columnVisibility: { 
    rejectionReasons: false,  // hidden by default
    ...layout.columnVisibility 
  },      density: 'compact',
      columnOrder: Array.isArray(layout.columnOrder) && layout.columnOrder.length ? layout.columnOrder : Array.from(new Set(['mrt-row-select', ...columns.map((c) => (c as any).id ?? (c as any).accessorKey).filter(Boolean)])),
    },
    state: { sorting, pagination, columnFilters, rowSelection, columnVisibility: layout.columnVisibility || {} },
      ...
      <div className="flex items-center p-2 w-full justify-between">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => {
              const currentFilter = customFilters.find((f: any) => f?.fieldId === '__duplicates_only');
              if (currentFilter) {
                setCustomFilters((prev: any) => prev.filter((f: any) => f?.fieldId !== '__duplicates_only'));
              } else {
                setCustomFilters((prev: any) => [...prev, { fieldId: '__duplicates_only', value: true, type: 'boolean', label: 'Show Duplicates Only' }]);
              }
            }}
          ...
          {duplicatesOnlyEnabled && (
            <span className="text-xs text-amber-600 dark:text-amber-400">
              {(() => {
                const duplicateLookup = buildApplicantDuplicateLookup(filteredApplicants as any[], currentUserId, { getCompanyId: (applicant: any) => getApplicantCompanyId(applicant, jobPositionMap) });
                const duplicateCount = Array.from(duplicateLookup.values()).filter((d) => d.isDuplicate === true).length;
        ...
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => setCustomFilterOpen(true)} className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-3 py-1 text-sm font-semibold text-white shadow-sm hover:bg-brand-600">
            Filter Settings
          </button>
        </div>
      </div>
    ...
    muiTableProps: { sx: { backgroundColor: isDarkMode ? '#24303F' : '#FFFFFF', tableLayout: 'auto', width: '100%', minWidth: `${tableMinWidth}px`, fontFamily: "'Cairo', Outfit, system-ui", fontSize: '0.82rem' } },
    muiTableContainerProps: { sx: { maxWidth: '100%', overflowX: 'auto' } },
    muiTableBodyCellProps: { sx: { backgroundColor: isDarkMode ? '#24303F' : '#FFFFFF', color: isDarkMode ? '#E4E7EC' : '#101828', borderColor: isDarkMode ? '#344054' : '#E4E7EC', display: 'flex', alignItems: 'center', fontSize: isLaptopViewport ? '0.76rem' : '0.8rem', padding: isLaptopViewport ? '5px 6px' : '6px 8px', whiteSpace: 'nowrap' } },
    muiTableHeadCellProps: { sx: { backgroundColor: isDarkMode ? '#1C2434' : '#F9FAFB', color: isDarkMode ? '#E4E7EC' : '#344054', fontWeight: 600, fontSize: isLaptopViewport ? '0.74rem' : '0.78rem', padding: isLaptopViewport ? '7px 6px' : '8px 8px', whiteSpace: 'nowrap' } },
    muiTableBodyRowProps: () => ({ sx: { cursor: 'default', backgroundColor: isDarkMode ? '#24303F' : '#FFFFFF', '&:hover': { backgroundColor: isDarkMode ? '#344054' : '#F9FAFB' } } }),
              ...
              <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-lg bg-brand-50 px-4 py-3 dark:bg-brand-900/20">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
                  ...
                  >
                    ...
                    {isExporting ? 'Exporting...' : `Export (${selectedApplicantCount})`}
                  </button>
                  <button
                    onClick={() => setShowBulkModal(true)}
                    disabled={isProcessing || selectedApplicantRecipients.length === 0}
                  ...
                  <button
                    ...
                    disabled={isSubmittingBulkInterview || selectedApplicantsForInterview.length === 0}
                  ...
                  >
                    {isSubmittingBulkInterview ? 'Scheduling...' : `Schedule Interviews (${selectedApplicantsForInterview.length})`}
                  </button>
                  <button
  ...
  buildInterviewEmailHtml={({ rawMessage }: any) => `<html><body>${rawMessage}</body></html>`}
  getJobTitle={() => ({ en: '' })}
  companyData={selectedApplicantCompany || null}
  applicant={{
    fullName: '{{candidateName}}',
    company: selectedApplicantCompany || (selectedApplicantCompanyId ? { _id: selectedApplicantCompanyId } : null),
    companyObj: selectedApplicantCompany || (selectedApplicantCompanyId ? { _id: selectedApplicantCompanyId } : null),
  }}
  bulkMode
...
/>
            ...
            <Modal isOpen={showBulkInterviewPreviewModal} onClose={() => setShowBulkInterviewPreviewModal(false)} className="max-w-5xl p-6">
              <div className="space-y-4">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Interview Email Preview ({bulkInterviewPreviewItems.length})</h2>
                <div className="max-h-[70vh] space-y-4 overflow-auto pr-1">
                  {bulkInterviewPreviewItems.map((item, index) => (
                    <div key={`${item.applicantId}-${index}`} className="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                      <div className="mb-2 text-sm font-semibold text-gray-800 dark:text-white/90">Applicant #{item.applicantNo} - {item.applicantName}</div>
                      ...
                      <iframe srcDoc={item.html} title={`Interview Preview ${item.applicantId}`} className="min-h-[360px] w-full rounded border-none bg-white" />
                    </div>
                  ))}
                </div>
                <div className="flex justify-end">
                  <button type="button" onClick={() => setShowBulkInterviewPreviewModal(false)} className="rounded-lg border border-stroke px-4 py-2 hover:bg-gray-100">Close</button>
                </div>
              </div>
            </Modal>

            <Modal isOpen={showBulkPreviewFallbackModal} onClose={() => setShowBulkPreviewFallbackModal(false)} className="max-w-3xl p-6">
              <div className="space-y-3">
                ...
                <div className="border rounded p-2 bg-white dark:bg-gray-800" style={{ maxHeight: '70vh', overflow: 'auto' }}>
                  <iframe srcDoc={bulkPreviewHtml} title="Bulk Interview Preview" className="w-full min-h-[480px] rounded border-none" />
                </div>
              </div>
            </Modal>
          </>
      ...
      {previewPhoto && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setPreviewPhoto(null)}>
          <div className="relative max-h-[90vh] max-w-[90vw] p-4">
            <button onClick={() => setPreviewPhoto(null)} className="absolute -top-2 -right-2 flex h-8 w-8 items-center justify-center rounded-full bg-white text-gray-700 shadow-lg hover:bg-gray-100">✕</button>
            <img src={previewPhoto} alt="Applicant photo preview" className="max-h-[85vh] max-w-full rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()} />
          </div>
        </div>
      )}
